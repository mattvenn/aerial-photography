...
def process(md_file):
    ...
    os.system('~/.cabal/bin/pandoc --mathjax --template=%s -c %s -H %s -A %s %s -o %s --variable date="%s" --variable pdf="%s"' % ( html_template, css, header, footer, tmp_file, html_file,date,pdf_file))
